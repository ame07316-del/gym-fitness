...
| قبل/بعد | صور قبل وبعد حقيقية (نفس الشخص/نفس الإضاءة) بسلايدر سحب + نبضة على المقبض |
...
1. **الدفع**: حط مفتاح البوابة في `.env` (`NEXT_PUBLIC_PAYMENT_PROVIDER=paymob|stripe` + `NEXT_PUBLIC_PAYMENT_SECRET=...`) وبدّل جسم `authorize()`/`confirmPayment()` في `app/lib/payment.ts` و`app/api/pay/route.ts` بنداءات البوابة — شكل الرد `PayResult` ثابت فالواجهة كلها من غير تعديل.
