...
افتح `/admin` من نفس الموقع لمتابعة الحجوزات والاشتراكات والإيرادات وتوزيع الباقات في لوحة واحدة. في الإنتاج الصفحة تحتاج `ADMIN_API_TOKEN` لأن مسارات الإدارة محمية من التسريب.

```bash
# .env.local — الإنتاج أو الاستخدام المحلي المحمي
ADMIN_API_TOKEN=ضع_توكن_قوي_هنا

# للمعاينة المحلية فقط، افتح اللوحة بدون إدخال توكن:
ADMIN_DEMO_MODE=true
```

`ADMIN_DEMO_MODE` يعمل فقط مع `NODE_ENV=development` ويعرض بيانات البيئة المحلية التجريبية؛ لا تفعّله على Vercel أو أي بيئة إنتاج. لا تستخدم `NEXT_PUBLIC_ADMIN_API_TOKEN` ولا تضع التوكن في Git. اللوحة تعرض آخر 25 حجزًا واشتراكًا، وتحتوي على بحث وتحديث وتسجيل خروج، بينما الـ API نفسه يظل صالحًا للاستخدام من أي لوحة خارجية.
