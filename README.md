...
افتح `/admin` من نفس الموقع لمتابعة الحجوزات والاشتراكات والإيرادات وتوزيع الباقات في لوحة واحدة. الصفحة نفسها موجودة داخل المشروع، لكنها لا تعرض البيانات إلا بعد إدخال `ADMIN_API_TOKEN` لأن مسارات الإدارة محمية من التسريب.

```bash
# .env.local — محليًا فقط، ثم أعد تشغيل Next.js
ADMIN_API_TOKEN=ضع_توكن_قوي_هنا
```

بعد تشغيل السيرفر، اكتب نفس التوكن في شاشة الدخول `/admin`. لا تستخدم `NEXT_PUBLIC_ADMIN_API_TOKEN` ولا تضع التوكن في Git. اللوحة تعرض آخر 25 حجزًا واشتراكًا، وتحتوي على بحث وتحديث وتسجيل خروج، بينما الـ API نفسه يظل صالحًا للاستخدام من أي لوحة خارجية.
