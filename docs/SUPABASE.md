...
افتح `/admin` لمتابعة الحجوزات والاشتراكات والإيرادات. أدخل نفس قيمة `ADMIN_API_TOKEN` الموجودة في بيئة السيرفر؛ الصفحة ترسلها في `Authorization: Bearer` فقط إلى مسارات الإدارة ولا تضعها في `NEXT_PUBLIC_*`.

إذا ظهر `401`، تأكد من ضبط التوكن ثم أعد تشغيل Next.js. وجود صفحة `/admin` لا يلغي حماية `GET /api/bookings` و`GET /api/subscribe`.
